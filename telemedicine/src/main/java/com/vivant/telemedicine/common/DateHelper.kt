...
object DateHelper {
    ...
    var DISPLAY_DATE_MMDDYYYY = "MM/dd/yyyy"
    ...
    val currentTimeAs_hh_mm: String
        get() {
    ...
    fun getTimeIn12HrFormatAmOrPm1(time: String): String {
        var timeInAmPm = ""
        try {
    ...
    val currentTimeIn12HrFormatAmOrPm: String
        get() {
            var timeInAmPm = ""
            try {
                val _24HourSDF = SimpleDateFormat("HH:mm", Locale.ENGLISH)
                val _12HourSDF = SimpleDateFormat("hh:mm a", Locale.ENGLISH)
                val _24HourDt = _24HourSDF.parse(currentTimeAs_hh_mm)
                timeInAmPm = _12HourSDF.format(_24HourDt!!)
            } catch (e: ParseException) {
                e.printStackTrace()
            }
    ...
    fun getTimeBeforeOrAfter(time: String,sourceFormat:String,destFormat:String,diff:Int): String {
        var timeInAmPm = ""
        try {
            val mil = SimpleDateFormat(sourceFormat,Locale.ENGLISH).parse(time)!!.time + diff
            timeInAmPm = SimpleDateFormat(destFormat,Locale.ENGLISH).format( Date(mil))
        } catch (e: ParseException) {
            e.printStackTrace()
        }
        return timeInAmPm
...
/*    fun calculateYears(dateAsString: String): Int {
    ...
    fun isDateAbove18Years(dateIn_ddMMMyyyy: String): Boolean {
        ...
        try {
            ...
            if (dateIn_ddMMMyyyy.contains("T")) {
                val datestring = dateIn_ddMMMyyyy.split("T".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()[0]
                val oldformat = SimpleDateFormat("yyyy-MM-dd", Locale.ENGLISH)
                date = oldformat.parse(datestring)
            } else {
                date = if (dateIn_ddMMMyyyy.length > 10) {
                    val oldformat = SimpleDateFormat("dd-MMM-yyyy", Locale.ENGLISH)
                    oldformat.parse(dateIn_ddMMMyyyy)
                } else {
                    val oldformat = SimpleDateFormat("MM-dd-yyyy", Locale.ENGLISH)
                    oldformat.parse(dateIn_ddMMMyyyy)
                }
    ...
    fun calculateAge(years: Int, months: Int, days: Int): String {
        ...
        try {
            ...
            val birthDate = sdf.parse(days.toString() + "/" + months + "/" + years) //Yeh !! It's my date of birth :-)
            //create calendar object for birth day
            val birthDay = Calendar.getInstance()
            birthDay.timeInMillis = birthDate.time
            //create calendar object for current day
        ...
        //Create new Age object
        utilities.printLogError("Age: $years $months $days")
        return if (years > 0) {
            "$years Years"
        } else if (years <= 0 && months > 0) {
            "$months Months"
